...
uses
  frameCadCli, pousada, FireDAC.Comp.Client,global, frameCadastro;
...
begin
    ...
    begin
      ...
      begin
        if texto = ' ' then
            add('select idcliente,nome from cliente where inativarCliente = 0')
        else
           add('select idcliente,nome from cliente where nome like ''%'+ texto+'%'' and inativarCliente = 0')
...
procedure TFrameCli.btnVoltaClick(Sender: TObject);
begin
  release;
