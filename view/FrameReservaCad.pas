...
begin
   ...
   begin
     ...
     begin
       if modi then
        mudanca_Reserva(EdtDataEnt.Text,EdtDatSai.Text,EdtNumPess.Text,edtNumCri.Text,edtValor.Text,id,reserv,id_reser)
       else
...
begin
  ...
  begin
    ...
    begin
      listaReser := pegarReserva(id_reserva,dat);
      if listaReser.Count > 0 then
      begin
      id_reserva :=  listaReser.Items[0].IdReserva;
      EdtDataEnt.Text := listaReser.Items[0].DataEntrada;
      EdtDatSai.Text  := listaReser.Items[0].DataSaida;
      edtValor.text   :=listaReser.Items[0].valorDiaria;
      EdtNumPess.Text  := listaReser.Items[0].numeroAdult.tostring();
      edtNumCri.Text   := listaReser.Items[0].NumeroCri.tostring();
      with tcliente.create(self) do
      begin
           nome  :=pegarClienteporId(listaReser.Items[0].Idcliente);
           lista:=pegarClientes;
           for I := 0 to lista.Count -1  do
               CmbNome.Items.Add(lista.Items[I].Nome);
           for I := 0 to lista.Count -1  do
           begin
                 if lista.Items[I].Nome = nome then
                     id:=I;
           end;
           CmbNome.ItemIndex:=id;
       end;
      end;
